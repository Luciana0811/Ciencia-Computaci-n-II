...
template<typename T>
void Lista<T>::orden_ascendete()
{
	T temp_data;
	Nodo<T> *aux_node = m_head;
	...
	while (aux_node) {
		...
		while (temp->siguiente) {
			...
			if (aux_node->dato > temp->dato) {
				temp_data = aux_node->dato;
				aux_node->dato = temp->dato;
				temp->dato = temp_data;
			}
		}
